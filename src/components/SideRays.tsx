 ...
 *
 * Upstream renders at up to 2x DPR, keeps its rAF running while the tab is
 * hidden and throws if WebGL is unavailable. The lifecycle here matches
 * LightRays.tsx instead: capped DPR, IntersectionObserver mount/teardown,
 * visibility-gated rAF, and a graceful bail when no context can be created.
 */
...
const SideRays = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      webglUnavailableRef.current = true;
      console.warn("SideRays disabled: unable to create WebGL context.", error);
      return;
    }
    ...
    const loop = (t: number) => {
      ...
      try {
        ...
        console.warn("SideRays rendering error:", error);
      }
    };
    ...
    cleanupFunctionRef.current = () => {
      ...
      try {
        ...
        console.warn("Error during SideRays WebGL cleanup:", error);
      }
