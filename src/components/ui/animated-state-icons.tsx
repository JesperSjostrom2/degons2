...
export function SendIcon({ size = 40, color = "currentColor", className, duration = 2600, isAnimating }: StateIconProps) {
  const auto = useAutoToggle(duration);
  const sent = isAnimating !== undefined ? isAnimating : auto;
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      <motion.g
        ...
        transition={{ duration: 0.5, delay: sent ? 0.5 : 0, ease: [0.32, 0.72, 0, 1] }}>
