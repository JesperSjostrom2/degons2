...
const FONT_FAMILY = 'var(--font-outfit), -apple-system, BlinkMacSystemFont, sans-serif'
const SUPER_W = 3500
const SUPER_H = 2500
const START_X = 80
const START_Y = 80
...
function CodeCard() {
  const lines = [
    { indent: 0, w: 60, c: '#dac5a7' },
    { indent: 1, w: 80, c: 'rgba(255,255,255,0.75)' },
    { indent: 1, w: 50, c: '#8b7355' },
    { indent: 2, w: 70, c: 'rgba(255,255,255,0.7)' },
    { indent: 2, w: 40, c: '#f5efe4' },
    { indent: 1, w: 30, c: 'rgba(255,255,255,0.45)' },
    { indent: 0, w: 20, c: 'rgba(255,255,255,0.45)' },
  ]

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace' }}>
      {lines.map((line, i) => (
        <div
          key={i}
          style={{
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div
        style={{
          ...
          height: 80,
          borderRadius: 20,
          background: `linear-gradient(135deg, ${accent} 0%, hsl(${hue}, 34%, 55%) 100%)`,
        }}
      />
    </div>
  )
  ...
  return (
    <div
      style={{
        width: '100%',
        height: '100%',
        background: `radial-gradient(circle at 30% 30%, hsl(${hue}, 38%, 58%) 0%, hsl(${(hue + 60) % 360}, 30%, 32%) 48%, #050505 100%)`,
      }}
    />
  )
}
...
export function InfiniteBentoPan({ panSpeed = 1, accentColor = '#dac5a7', speed = 1, className }: InfiniteBentoPanProps) {
    ...
    >
      <div
        style={{
          ...
          width: SUPER_W,
          height: SUPER_H,
          transform: `translate(${-px - START_X}px, ${-py - START_Y}px)`,
          willChange: 'transform',
        }}
      >
        {CARDS.map((card, i) => (
          <Card key={i} card={card} accent={accentColor} index={i} frame={frame} />
        ))}
      </div>
      <div
        style={{
          position: 'absolute',
          inset: 0,
          background: 'radial-gradient(circle at center, transparent 25%, rgba(0,0,0,0.72) 78%, rgba(0,0,0,0.9) 100%)',
          pointerEvents: 'none',
        }}
      />
    </div>
